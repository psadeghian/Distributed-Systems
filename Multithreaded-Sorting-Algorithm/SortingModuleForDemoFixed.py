...
class Sorter:
    ...
    def __sortListForMultiProcessing(self, lyst):
        lyst.sort()
        self.manager_list.append(lyst)
    ...
    def mergeSortByMultiProcessing(self, lyst, numOfProcesses):
        ...
        for i in range(numOfProcesses):
            p = multiprocessing.Process(target=self.__sortListForMultiProcessing, args=(chunkedList[i],))
            processList.append(p)
            p.start()
        for p in processList:
            p.join()
        for item in heapq.merge(*self.manager_list):
            mergedList.append(item)
        return mergedList
